...
export function mountApp(root: HTMLElement): void {
  ...
  function renderRoutine(): string {
    ...
    return `
      <section class="panel stack">
        ...
        <p class="footer-note">여유루틴 v${escapeHtml(import.meta.env.VITE_APP_VERSION || '0.4.1')} · 로컬 전용</p>
      </section>`
  }
  ...
  function renderSettings(): string {
    ...
    return `
      <section class="panel stack">
        ...
        <p class="footer-note">여유루틴 v${escapeHtml(import.meta.env.VITE_APP_VERSION || '0.4.1')} · 습관 가이드</p>
      </section>`
  }
  ...
  function render(): void {
    root.innerHTML = `
      <header class="app-header">
        ...
        <p class="version-chip" aria-label="app version">앱 버전 v${escapeHtml(import.meta.env.VITE_APP_VERSION || '0.4.1')}</p>
      </header>
      ${renderTabs()}
      ${panelBody()}
      ${renderPaywallModal()}
  ...
  async function refreshAppCache(): Promise<void> {
    ...
    location.href = '/?v=0.4.1&_=' + Date.now()
  }
